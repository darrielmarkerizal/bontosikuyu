...
export default function TambahPariwisataPage() {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const response = await axios.get("/api/travel-categories", {
        ...
        if (response.data.success) {
          ...
          // Remove the default category setting - let user choose
        }
      } catch (error) {
        ...
        toast.error("Gagal memuat data kategori", {
          description: "Tidak dapat memuat data kategori wisata",
        });
      } finally {
        setLoading(false);
      }
        ...
        <div className="flex items-center gap-3 sm:gap-4">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-sm sm:text-base text-muted-foreground mt-1">
              Tambah destinasi wisata baru untuk website Desa Laiyolo Baru
            </p>
          </div>
        </div>
